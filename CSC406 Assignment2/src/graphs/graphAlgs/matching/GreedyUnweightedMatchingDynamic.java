...
public class GreedyUnweightedMatchingDynamic extends MatchingAlgorithm {
	...
	public void runAlgorithm(AdjacencyListGraph<Point2D.Double, Double> g) {
		...
		while (!heap.isEmpty()) {
			...
			for (Edge b : g.incidentEdges(currVertex)) // goes through all end
														// vertices to try and
														// get their degree
			{
				currIncVertex = g.opposite(currVertex, b); // gets the vertex
															// using the current
															// incident edge and
															// saves it
				currInc = b;
				...
				if (temp < bestSoFar && (currIncVertex.get(VERTEX_STATUS) == FREE)) {
					...
					bestSoFar = temp; // saves best vertie and lowest degree so
										// far
					bestEdge = currInc;
				}
			}
			if (currVertex.get(VERTEX_STATUS) == FREE && g.degree(currVertex) != 0 && bestIncVertex != null) {
